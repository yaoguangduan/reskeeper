...
func TestPson(t *testing.T) {
	...
	ss := `{age:12,name:alen,addr:{name:BJ,x:43.12,y:98.41},assistants:[{name:AA,level:99},{name:CC,direction:health}],actonFlow:{1998:eat,2020:fly},papers:[{id:81723,desc:woigwieqw},{id:9872312,desc:owughqiweuyqw}]}`
	manager := pbgen.Manager{}
	m := dynamicpb.NewMessage(manager.ProtoReflect().Descriptor())
	Decode("", m, s)
	fmt.Println(m)
	mm := dynamicpb.NewMessage(manager.ProtoReflect().Descriptor())
	Decode("", mm, ss)
	fmt.Println(mm)
}
